...
func getVideoSizeMakeThumbnail(temp string, todir string, toname string, info map[string]interface{}) {
	...
	cmd := exec.Command(ffmpegpath, "-i", temp, "-ss", fmt.Sprintf("%s", duration), "-vframes", "1", "-q:v", "8", "-f", "image2", screenshotpath, "-y")
	stdout, err := cmd.StdoutPipe()
	check(0, "cmd.StdoutPipe", err)
	defer stdout.Close()
	err = cmd.Start()
...
func scaleVideo(temp string, todir string, toname string, info map[string]interface{}, tofile []interface{}) {
	...
	// println(temp)
	// println(tofilepath)
	// cmd := exec.Command("ffmpegpath", "-i", temp, "-i", "logo.png", "-strict", "-2", "-filter_complex", vf, "-b:v", bitrate, tofilepath, "-hide_banner", "-y")
