...
func strfind(file string) bool {
	...
	for {
		...
		if newLine != linestr {
			...
			fmt.Println("　行" + strconv.Itoa(lineindex) + "原文: " + linestr)
			if replace != "" || ls == false {
				fmt.Println("　行" + strconv.Itoa(lineindex) + "改后: " + newLine)
			}
			isok = true
		}
		lineindex++
...
func main() {
	...
	flag.StringVar(&replace, "r", "", "要替换的字符串（不加则视为 -l ）")
	flag.BoolVar(&subdir, "s", false, "扫描子目录")
	flag.StringVar(&extensionstr, "e", "", "扩展名列表，英文逗号分隔，不加 '.' ，例如 \"php,py\" ")
	flag.BoolVar(&verbose, "v", false, "显示详细搜索过程（慢）")
	flag.BoolVar(&ls, "l", false, "不进行替换操作，只显示。")
	flag.Parse()
	var fgstr = "==========="
	fmt.Println(fgstr + fgstr + fgstr)
	fmt.Println("雅诗批量文件字符串替换工具 v1.1")
	fmt.Println(fgstr + fgstr + fgstr)
	...
	fmt.Println("扩展名过滤: " + extensionstr)
	...
	if find == "" {
		fmt.Println("错误: 必须指定 -f 来决定要查找替换的内容。")
