...
**發送格式：** `[tag|]層級|令牌`，層級為 0-3，控制回覆的資訊豐富度。

```
發送格式： 層級|令牌        →  回覆： 依層級決定
          tag|層級|令牌     →  回覆： tag|依層級決定
```
...
| 層級 | 用途 | 回覆內容 | 適用場景 |
|------|------|----------|----------|
| `0` | 精簡 | 整數錯誤代碼 | 僅需判斷通過／不通過，最小傳輸量 |
| `1` | 系統 | `{"success":bool}` + `message`（失敗時） | 需要知道驗證是否成功，不需令牌內容 |
| `2` | 系統＋claims | 層級 1 + `username`、`app`、`sub`、`iat`、`exp` | 需要使用者身分與時效資訊 |
| `3` | 完整 | 層級 2 + `user` 詳細資料 | 需要所有資訊（不含多餘的 DB 查詢，僅驗證令牌） |
...
#### 層級 2 — 系統＋token claims

回覆成功時包含令牌中的全部標準 claims 資訊。
...
```json
{"success":true,"username":"user","app":"app","sub":"user","iss":"/auth/login","iat":"...","nbf":"...","exp":"...","jti":"abc123..."}
```
...
回傳完整 `verifyResponse` JSON，包含從令牌解密出的所有資訊（含上游資料庫的使用者詳細資料 `user` 物件）。

**回覆 — 核實成功：**

```json
{"success":true,"username":"user","app":"app","sub":"user","iat":"...","exp":"...","user":{"id":1,"username":"user",...}}
```
...
可在層級前面附加任意 tag，回覆時以 `tag|` 前綴帶回，方便呼叫方匹配請求與回覆。
...
|----------|------|-----|----------|
| `0\|v2.local...` | 0 | (無) | `0` |
| `2\|v2.local...` | 2 | (無) | `{"success":true,"username":"user",...}` |
| `req1\|0\|v2.local...` | 0 | `req1` | `req1\|0` |
| `req1\|2\|v2.local...` | 2 | `req1` | `req1\|{"success":true,...}` |
| `abc-123\|3\|v2.local...` | 3 | `abc-123` | `abc-123\|{"success":true,...,"user":{...}}` |
...
resp, err := natsClient.Request("auth.token.verify",
    tag+"|2|v2.local.FcG...", 5*time.Second)
if err != nil {
    return
}
...
async def main():
    ...
    # 層級 0：精簡模式
    resp = await nc.request("auth.token.verify", b"0|v2.local.FcG...", timeout=5)
    code = int(resp.data)
    print("有效" if code == 0 else f"無效，代碼: {code}")

    # 層級 2：系統＋claims
    resp = await nc.request("auth.token.verify", b"2|v2.local.FcG...", timeout=5)
    result = json.loads(resp.data)
    print(f"使用者: {result['username']}")

    # 層級 2 + tag
    resp = await nc.request("auth.token.verify", b"my-req-1|2|v2.local.FcG...", timeout=5)
...
| 欄位 | 層級 0 | 層級 1 | 層級 2 | 層級 3 | 說明 |
|------|--------|--------|--------|--------|------|
| 整數代碼 | 是 | — | — | — | 0=有效, 1-4=錯誤 |
| `success` | — | 是 | 是 | 是 | 核實是否成功 |
| `message` | — | 失敗時 | 失敗時 | 失敗時 | 錯誤原因說明 |
| `username` | — | — | 成功時 | 成功時 | 令牌所屬使用者名稱 |
| `app` | — | — | 成功時 | 成功時 | 令牌對應的應用程式識別名稱 |
| `sub` | — | — | 成功時 | 成功時 | 令牌主體 |
| `iss` | — | — | 成功時 | 成功時 | 簽發者（Issuer） |
| `iat` | — | — | 成功時 | 成功時 | 令牌簽發時間（ISO 8601） |
| `nbf` | — | — | 成功時 | 成功時 | 令牌生效時間（ISO 8601） |
| `exp` | — | — | 成功時 | 成功時 | 令牌到期時間（ISO 8601） |
| `jti` | — | — | 成功時 | 成功時 | 令牌唯一識別碼（JWT ID） |
| `user` | — | — | — | 成功時 | 使用者詳細資料（含 id、nickname、group、permissions 等） |
...
使用層級數字 0-3 控制回覆資訊量，可在層級前附加選填 tag。
...
|------|------|-----|----------|
| `0\|v2.local...` | 0（精簡） | 無 | `0` |
| `1\|v2.local...` | 1（系統） | 無 | `{"success":true}` |
| `2\|v2.local...` | 2（系統+claims） | 無 | `{"success":true,"username":"user",...}` |
| `3\|v2.local...` | 3（完整） | 無 | `{"success":true,"username":"user",...,"user":{...}}` |
| `req1\|0\|v2.local...` | 0（精簡） | `req1` | `req1\|0` |
| `req1\|2\|v2.local...` | 2（系統+claims） | `req1` | `req1\|{"success":true,...}` |
...
僅回傳 success 與失敗時的 message，不含任何令牌內容。
...
#### 層級 2（系統＋claims）— 回傳 JSON 含令牌資訊

回傳基本狀態及令牌中的使用者身分與時效資訊。
...
#### 層級 3（完整）— 回傳 JSON 含使用者詳細資料

回傳完整 `verifyResponse` JSON，包含 `user` 物件（id、nickname、group、permissions 等）。
...
```json
{"success":true,"username":"user","app":"app","sub":"user","iat":"...","exp":"...","user":{"id":1,"username":"user","nickname":"...",...}}
```
...
在層級前附加任意 tag，回覆時以 `tag|` 前綴帶回。

```
發送:  req-42|0|v2.local...   →   回覆:  req-42|0
發送:  req-42|2|v2.local...   →   回覆:  req-42|{"success":true,...}
```

> **注意：** 層級 0 回傳整數代碼字串（非 JSON）。層級 1-3 回傳 JSON。均無 HTTP 狀態碼。
...
此介面為直接 NATS Request/Reply，**無 HTTP 狀態碼**。資訊層級由發送時的層級數字（0-3）決定。
...
**層級 2（系統＋claims）— 回傳 JSON：**

| success | message | 其他欄位 | 說明 |
|---------|---------|----------|------|
| `true` | (無) | `username`, `app`, `sub`, `iss`, `iat`, `nbf`, `exp`, `jti` | 驗證通過，含全部標準 claims |
| `false` | 錯誤描述 | (無) | 驗證失敗 |
...
| success | message | 其他欄位 | 說明 |
|---------|---------|----------|------|
| `true` | (無) | `username`, `app`, `sub`, `iat`, `exp`, `user` | 驗證通過，含完整使用者資訊 |
| `false` | 錯誤描述 | (無) | 驗證失敗 |
